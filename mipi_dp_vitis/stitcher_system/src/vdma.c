...
u8 cam_frame_buffer[BUFFERSIZE * DISPLAY_NUM_FRAMES * 2] __attribute__((__aligned__(256)));

u8 *cam0_frame = cam_frame_buffer + 0;
u8 *cam1_frame = cam_frame_buffer + BUFFERSIZE * DISPLAY_NUM_FRAMES;
...
int vdma_write_init(UINTPTR BaseAddress, XAxiVdma *p_vdma,
  ...
  xil_printf("vdma at %p initialized\n", BaseAddress);
