...
int gamma_lut_init(u32 BaseAddress) {
	XV_gamma_lut gamma_lut;
	XV_gamma_lut_Config *gamma_lut_config;

	if ( (gamma_lut_config = XV_gamma_lut_LookupConfig(BaseAddress)) == NULL) {
	...
	XV_gamma_lut_Set_HwReg_width(&gamma_lut, SRC_WIDTH);
	XV_gamma_lut_Set_HwReg_height(&gamma_lut, SRC_HEIGHT);
